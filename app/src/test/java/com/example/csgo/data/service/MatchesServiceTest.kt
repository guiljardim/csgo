...
class MatchRemoteDataSourceTest {
    ...
    @Test
    fun `should fetch matches correctly given 200 response`() {
        ...
        runBlocking {
            val actual = service.getMatchesSync("", "", 2, 2, "")
    ...
    @Test
    fun `should not fetch matches correctly given 400 response`() {
        val expected = getListOfMatchRemote()
        val response = MockResponse()
    ...
    @Test
    fun `should fetch data matches correctly`() {
        val expected = getListOfMatchRemote()
        val response = MockResponse()
            .setResponseCode(HttpURLConnection.HTTP_OK)
            .setBody(Gson().toJson(expected))
        ...
        runBlocking {
            ...
            assertEquals(Gson().toJson(expected), Gson().toJson(actual.body()))
        }
    }
}
